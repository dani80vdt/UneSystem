	 ...
	 { 
		...
		{   
			...
			if ($.mobile.activePage.is("#pag1"))
				$.mobile.changePage("#pag2", { transition: "none", reverse: false, changeHash: false});
			else if ($.mobile.activePage.is("#pag2"))
				$.mobile.changePage("#pag3", { transition: "none", reverse: false, changeHash: false});	
		}
		return false;         
	});
	...
	{   
		...
		{      
			...
			if ($.mobile.activePage.is("#pag2"))
				$.mobile.changePage("#pag1", { transition: "none", reverse: true, changeHash: false});	
			else if ($.mobile.activePage.is("#pag3"))
				$.mobile.changePage("#pag2", { transition: "none", reverse: true, changeHash: false});			
		}
		return false;            
	});
...
function showPage1(u,p)
{
	...
	$.mobile.changePage("#pag1", {transition: "none",reverse: false, changeHash: false});
	$.mobile.hidePageLoadingMsg();
	curForm="main";
}
...
function handleLogin(u,p) 
{
	...
	else 
	{
		...
		$.mobile.hidePageLoadingMsg();
		$.mobile.changePage("#loginForm", { transition: "none", reverse: false, changeHash: false});
		var form = $("#loginForm");
		$("#username", form).val();
		var p = $("#password", form).val();	
    }
	...
	if (tableDevices!=undefined)
	{
		...
		for (var i=0; i< G_ListData.length; i++)
		{
			...
			var html ="  <tr> " + 
                ...
                " <img src='./images/panel2.png' id='imgDevice1' class='imgDevice1' /> " +
				" </a> " + 
                " </td> " +
                " <td> " +                
                " <table> " +
...
{
	$.mobile.changePage("#pag0", { transition: "none", reverse: false, changeHash: false});
}
